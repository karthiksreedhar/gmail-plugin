      ...
      while (true) {
        ...
        try {
          const threadResp = await withTimeout(
            gmailClient.users.threads.get({ userId: 'me', id: threadId, format: 'minimal' }),
            AUTO_SYNC_GMAIL_CALL_TIMEOUT_MS,
            'gmail_thread_get_backfill'
          );
...
`;
    ...
    if (exampleSelection.selected.length === 0) {
      prompt += `(No previous email responses are available. Write a reasonable, concise, professional reply to the new email.)\n`;
    }
